...
def get_clean_correlation(p_pre, p_post, warp_matrix, dataset_name):
    ...
    # Use INTER_CUBIC to preserve peak sharpness during sub-pixel shifts!
    ...
    print(f"{dataset_name} | CLEAN Corr (Cubic Interpolation): {corr:.4f} | Valid pillars: {np.sum(clean_mask)}")
...
get_clean_correlation(p_pre, p_post, M_260704, "260704 (Control)")
...
get_clean_correlation(p_pre, p_post, M_p50, "p50 Full Exp")
