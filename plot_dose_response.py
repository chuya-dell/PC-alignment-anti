...
EXCLUDE_MAP = {
    ...
    '260707_sam_p100_1': ['8', '6'],     # No.8: 0 M(疑問), No.6: 10 fM (焦点ズレ減光)
    '260707_sam_p100_2': ['7']           # No.7: 1 fM (Post時基板剥離)
}

# 100% exact mappings from user prompt
mapping_p200 = {
    '13': ('0 M (Blank)', 0.0),
    '6':  ('1 fM', 1e-15),
    '10': ('10 fM', 1e-14),
...
for name, excel_path, mapping in datasets:
    ...
    g = df_valid.groupby(['濃度(M)', '濃度表記', '評価パターン']).agg(
        mean_5s=(col_5s, lambda x: np.mean(x)*100 if np.mean(x)<1.0 else np.mean(x)),
        sem_5s=(col_5s, lambda x: (np.std(x, ddof=1)*100 if np.mean(x)<1.0 else np.std(x, ddof=1))/np.sqrt(len(x)) if len(x)>1 else 0),
        mean_delta=('BG比輝度変化 (ΔMean)', 'mean'),
        sem_delta=('BG比輝度変化 (ΔMean)', lambda x: np.std(x, ddof=1)/np.sqrt(len(x)) if len(x)>1 else 0)
    ...
    ax1.set_title(f'{name} (Exact Mapping): 5-Sigma Ratio [%]', fontsize=12, fontweight='bold')
    ...
    ax2.set_title(f'{name} (Exact Mapping): Delta Mean Intensity', fontsize=12, fontweight='bold')
