...
if __name__ == '__main__':
    ...
    print("\n--- Absolute Time Drift (All FOVs) ---")
    sp, _, _, pp, _ = stats.linregress(df['time_rel'], df['pillar_delta'])
    sb, _, _, pb, _ = stats.linregress(df['time_rel'], df['bg_delta'])
    sf, _, _, pf, _ = stats.linregress(df['time_rel'], df['fwhm'])
    print(f"Pillar Delta Trend: {sp:.4f}/min (p={pp:.4f})")
    print(f"BG Delta Trend:     {sb:.4f}/min (p={pb:.4f})")
    print(f"FWHM Trend:         {sf:.4f}/min (p={pf:.4f})")
