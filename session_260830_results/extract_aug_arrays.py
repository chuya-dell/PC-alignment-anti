...
datasets = ['260826-p50-sam', '260829-p50-sam']
...
def process_fov(args):
    ...
    deltas = (int0 - int1) / int0 * 100
    
    return {'dataset': dataset, 's': s, 'f': f, 'corr': corr, 'deltas': deltas}

if __name__ == '__main__':
    args = [(d, s, f) for d in datasets for s in range(1, 9) for f in range(1, 9)]
    results = []
    print("Batch processing August arrays...")
    with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
        for res in executor.map(process_fov, args):
            if res: results.append(res)
            
    with open(os.path.join(out_dir, 'aug_arrays.pkl'), 'wb') as f:
