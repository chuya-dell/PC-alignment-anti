...
if __name__ == '__main__':
    ...
    df = df[df['corr'] >= 0.70].copy()
    df['time_rel'] = (df['timestamp'] - df['timestamp'].min()) / 60
    
    with open(os.path.join(out_dir, 'drift_results_control.pkl'), 'wb') as f:
        pickle.dump(df, f)
        
    print("\n--- Control Sample Averages ---")
    print("Substrate | Cond | Pillar Delta (%) | BG Delta (%) | FWHM (px)")
    cond_map = {1: 'Wash(③)', 2: 'Inc(①)', 3: 'Blow(②)', 4: 'Wash(③)', 5: 'Inc(①)'}
    for s in range(1, 6):
        s_df = df[df['s'] == s]
        if len(s_df) > 0:
            print(f"Sub {s} | {cond_map[s]} | {s_df['pillar_delta'].mean():.3f} | {s_df['bg_delta'].mean():.3f} | {s_df['fwhm'].mean():.3f}")
            
    s1 = df[df['s']==1]['pillar_delta'].mean()
    s4 = df[df['s']==4]['pillar_delta'].mean()
    print(f"\nCondition 3 (Wash) Time Check: Sub1={s1:.3f}%, Sub4={s4:.3f}%")
